...
import java.sql.*;
public class Withdrawal extends Panel implements ActionListener
{
	...
	JInternalFrame jif;
	Withdrawal(JInternalFrame Frame1)
	{
		jif=Frame1;
		setLayout(null);
		...
		if(ob==jb1)
		{
			...
			if((jb1.getLabel()).equals("OK"))
			{
				...
				try
				{
					...
					if(rs.next())
					{
						JOptionPane.showMessageDialog(null,"You have entered correct details","Withdrawal",1);
						remove(jb1);
						remove(jb2);
						remove(jt14);
						remove(jt15);
						...
						jl25=new Label("Enter money you want to Withdraw");
						jl25.setBounds(50,100,200,20);
						add(jl25);		
						jt16=new TextField();
						jt16.setBounds(260,100,200,25);
						add(jt16);	
						jb1.setLabel("Withdraw");
			...
			else
			{
				try
				{
					...
					if(a>b)
					{
						JOptionPane.showMessageDialog(null,"The available Balance is less than Withdrawing amount","Withdrawal",0);
					}
					else
					{
